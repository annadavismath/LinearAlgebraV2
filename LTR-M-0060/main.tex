...
\author{Anna Davis \and Paul Zachlin} \title{LTR-0060: Isomorphic Vector Spaces} \license{CC-BY 4.0}
...
\subsection*{Some Results Concerning One-to-One Transformations and Onto Transformations}
...
\begin{definition} Let $V$ and $W$ be vector spaces.  If there exists an isomorphism from $V$ onto $W$ we say that $V$ and $W$ are \dfn{isomorphic} and write $V\cong W$.
\end{definition}
...
The vector space $\mathbb{P}^2$ of polynomials of degree less than or equal to two is isomorphic to $\RR^3$.
\end{example}

\begin{explanation}
Recall that we defined $$\mathbb{P}^2=\left\{ax^2+bx+c : a,b,c \in \mathbb{R} \right\},$$ and we showed in Example \ref{ex:dimofP} of VSP-M-0060 that a basis for $\mathbb{P}^2$ is given by the three polynomials $\left\{1,x,x^2\right\}$.  Since $\mbox{dim}\,\mathbb{P}^2=3=\mbox{dim}\,\RR^3$, we conclude that $\mathbb{P}^2$ is isomorphic to $\RR^3$.
\end{explanation}
...
\begin{exploration}\label{init:coordmapping} In this problem we will explore an isomorphism between an $n$-dimensional vector space and $\RR^n$.
