...
The two properties of matrix multiplications translate into analogous properties of matrix transformations.  Suppose $T:\RR^n\longrightarrow\RR^m$ is a matrix transformation, then for all vectors $\vec{v}$, $\vec{w}$ in $\RR^n$ and all real number constants $k$ in $\RR$,
