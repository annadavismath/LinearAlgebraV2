...
Even by looking at a handful of vectors it is difficult to tell what the transformation actually accomplishes.  This is why sometimes looking at points instead of vectors can be beneficial.  If we consider every point in the left grid below as a tip of a vector, we can apply the transformation to each point to obtain the grid on the right.
...
We immediately see that the $y$ component of the vector remains unchanged.  We also see that the $x$ component increases (or decreases) by an increment that depends on $y$.  When considering $T$ as a transformation acting on points, we see that points located 1 unit above the $x$-axis, get shifted to the right by 0.5.  Points located 2 units above, get shifted to the right by 1. The higher the point, the greater the shift.  Points with negative $y$-coordinates get shifted to the left. In this fashion $T$ shears the entire plane.
 ...
 Now that we have seen the effect of functions defined via matrix multiplication, we can better appreciate the term \dfn{transformation}, as such functions distort the domain and the shapes located in it.
...
\draw[thin,gray!40] (-1,-3) grid (4,3);
  \draw[<->] (-1,0)--(4,0);
   \draw[<->] (0,-3)--(0,3);
\draw[line width=1.5pt,-stealth](0,0)--(2,1);
 \node[] at (1, 2)   (b) {$\begin{bmatrix}2\\1\end{bmatrix}$};
      \end{tikzpicture}
   \quad\quad
\begin{tikzpicture}[scale=0.5]
\draw[thin,gray!40] (-1,-3) grid (4,3);
  \draw[<->] (-1,0)--(4,0);
   \draw[<->] (0,-3)--(0,3);
\draw[line width=1.5pt,-stealth](0,0)--(4,2);
\node[] at (1, 2)   (b) {$2\begin{bmatrix}2\\1\end{bmatrix}$};
      \end{tikzpicture}
   \quad\quad   
\begin{tikzpicture}[scale=0.5]
\draw[thin,gray!40] (-1,-3) grid (6,3);
  \draw[<->] (-1,0)--(6,0);
   \draw[<->] (0,-3)--(0,3);
\draw[line width=1.5pt,-stealth,red](0,0)--(6,-2);
\node[red] at (2, -2)   (b) {$T\left(2\begin{bmatrix}2\\1\end{bmatrix}\right)$};
      \end{tikzpicture}   
\end{center}
 The second sequence of three diagrams corresponds to the right-hand side.  
 \begin{center}
\begin{tikzpicture}[scale=0.5]
\draw[thin,gray!40] (-1,-3) grid (4,3);
  \draw[<->] (-1,0)--(4,0);
...
\draw[thin,gray!40] (-1,-3) grid (4,3);
  \draw[<->] (-1,0)--(4,0);
   \draw[<->] (0,-3)--(0,3);
\draw[line width=1.5pt,-stealth, red](0,0)--(3,-1);
\node[red] at (1.5, -2)   (b) {$T\left(\begin{bmatrix}2\\1\end{bmatrix}\right)$};
      \end{tikzpicture}
   \quad\quad   
\begin{tikzpicture}[scale=0.5]
\draw[thin,gray!40] (-1,-3) grid (6,3);
  \draw[<->] (-1,0)--(6,0);
   \draw[<->] (0,-3)--(0,3);
\draw[line width=1.5pt,-stealth,red](0,0)--(6,-2);
\node[red] at (2, -2)   (b) {$2T\left(\begin{bmatrix}2\\1\end{bmatrix}\right)$};
      \end{tikzpicture}   
\end{center}
In the first sequence of steps, we double the vector first, then apply the transformation.  In the second sequence, we apply the transformation directly to $\begin{bmatrix}2\\1\end{bmatrix}$, then double the image.  Both approaches produce the same final result, $\begin{bmatrix}6\\-2\end{bmatrix}$.
\end{exploration}
...
For the left-hand side, we start by finding the sum of the two vectors, then we apply the transformation to the sum.
\begin{center}
\begin{tikzpicture}[scale=0.8]
\draw[thin,gray!40] (-2,-2) grid (3,3);
  \draw[<->] (-2,0)--(3,0);
   \draw[<->] (0,-2)--(0,3);
\draw[line width=1.5pt,-stealth](0,0)--(-1,-2);
\draw[line width=1.5pt,-stealth](0,0)--(2,1);
\filldraw[blue, opacity=0.3](0,0)--(-1,-2)--(1,-1)--(2,1)--cycle;
\draw[line width=1.5pt,-stealth, red](0,0)--(1,-1);
 \node[red] at (2, -1)   (b) {$\begin{bmatrix}1\\-1\end{bmatrix}$};
      \end{tikzpicture}
   \quad\quad
\begin{tikzpicture}[scale=0.8]
\draw[thin,gray!40] (-2,-2) grid (3,3);
  \draw[<->] (-2,0)--(3,0);
   \draw[<->] (0,-2)--(0,3);
\draw[line width=1.5pt,-stealth, red](0,0)--(2,3);
 \node[red] at (2.5, 1.8)   (b) {$T\left(\begin{bmatrix}1\\-1\end{bmatrix}\right)$};
      \end{tikzpicture}
  \end{center}
For the right-hand side, we apply the transformation to each vector separately, then find the sum of the images. 
 \begin{center}
\begin{tikzpicture}[scale=0.8]
\draw[thin,gray!40] (-2,-2) grid (3,3);
  \draw[<->] (-2,0)--(3,0);
...
%\filldraw[blue, opacity=0.3](0,0)--(-1,-2)--(1,-1)--(2,1)--cycle;
%\draw[line width=1.5pt,-stealth, red](0,0)--(1,-1);
 \node[] at (2, 1.8)   (b) {$\begin{bmatrix}2\\1\end{bmatrix}$};
 \node[] at (-1.5, -1)   (b) {$\begin{bmatrix}-1\\-2\end{bmatrix}$};
      \end{tikzpicture}
   \quad\quad
\begin{tikzpicture}[scale=0.8]
\draw[thin,gray!40] (-2,-2) grid (3,3);
  \draw[<->] (-2,0)--(3,0);
   \draw[<->] (0,-2)--(0,3);
   \filldraw[blue, opacity=0.3](0,0)--(1,0)--(2,3)--(1,3)--cycle;
\draw[line width=1.5pt,-stealth, red](0,0)--(2,3);
 \node[] at (1.8, -0.8)   (b) {$T\left(\begin{bmatrix}-1\\-2\end{bmatrix}\right)$};
 \node[] at (-0.5, 2)   (b) {$T\left(\begin{bmatrix}2\\1\end{bmatrix}\right)$};
 \draw[line width=1.5pt,-stealth](0,0)--(1,0);
 \draw[line width=1.5pt,-stealth](0,0)--(1,3);
 \node[red] at (4, 2)   (b) {$T\left(\begin{bmatrix}-1\\-2\end{bmatrix}\right)+T\left(\begin{bmatrix}2\\1\end{bmatrix}\right)$};
       \end{tikzpicture}
  \end{center}
...
In general, any transformation that satisfies (\ref{eq:matrixTransProp}) is called a \dfn{linear transformation}.  By Theorem \ref{th:matrixTransProp}, all matrix transformations are linear.
...
Example \ref{ex:imageOfBasisVectors} illustrates that a linear transformation $T:\RR^n\longrightarrow\RR^m$ is \dfn{completely determined} by where it maps the standard unit vectors.  This is true because we can express every vector $\vec{v}$ in $\RR^n$ as a linear combination of the standard unit vectors, then use (\ref{eq:matrixTransProp}) to find the image of $\vec{v}$.  The following GeoGebra interactive illustrates this point.
