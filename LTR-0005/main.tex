...
Try each of the following matrices to determine what each transformation accomplishes.
...
The two properties of matrix multiplications translate into analogous properties of matrix transformations.  Suppose $T:\RR^n\longrightarrow\RR^m$ is a matrix transformation, then for all vectors $\vec{v}_1$, $\vec{v}_2$ in $\RR^n$ and all real number constants $k$ in $\RR$,
\begin{equation}\label{eq:matrixTransProp1} 
T(k\vec{v}_1)=kT(\vec{v}_1)
    \end{equation}
    \begin{equation}\label{eq:matrixTransProp2} T(\vec{v}_1+\vec{v}_2)=T(\vec{v}_1)+T(\vec{v_2})
    \end{equation}

In general, any transformation that satisfies (\ref{eq:matrixTransProp1},\ref{eq:matrixTransProp2}) is called a \dfn{linear transformation}.  As we have just found, all matrix transformations are linear.
