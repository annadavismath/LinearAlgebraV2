...
% \begin{example}\label{ex:detinverseformula}
% Let $A=\begin{bmatrix}a&b\\c&d\end{bmatrix}$.  Show that $$A^{-1}=\frac{1}{ad-bc}\begin{bmatrix}d&-b\\-c&a\end{bmatrix}$$
% whenever $ad-bc\neq 0$.
% \begin{explanation}
% We will compute two products.
% $$\frac{1}{ad-bc}\begin{bmatrix}d&-b\\-c&a\end{bmatrix}\begin{bmatrix}a&b\\c&d\end{bmatrix}=\frac{1}{ad-bc}\begin{bmatrix}ad-bc&bd-bd\\-ac+ac&-bc+ad\end{bmatrix}=\begin{bmatrix}1&0\\0&1\end{bmatrix}=I$$
% \begin{align*}
% &\begin{bmatrix}a&b\\c&d\end{bmatrix}\left(\frac{1}{ad-bc}\begin{bmatrix}d&-b\\-c&a\end{bmatrix}\right)=\frac{1}{ad-bc}\begin{bmatrix}a&b\\c&d\end{bmatrix}\begin{bmatrix}d&-b\\-c&a\end{bmatrix}\\
% &=\frac{1}{ad-bc}\begin{bmatrix}ad-bc&-ab+ab\\cd-cd&-bc+ad\end{bmatrix}=\begin{bmatrix}1&0\\0&1\end{bmatrix}=I
% \end{align*}
% \end{explanation}
% \end{example}
% The result in Example \ref{ex:detinverseformula} is significant enough to be set aside as a formula.
...
%\section*{Row-reduction Method for Computing Matrix Inverses}
...
First, suppose that $\textnormal{rref}(A)=I$, then we can use elementary row operations to carry each $[A|\vec{e}_i]$ to its reduced row-echelon form.
...
\begin{corollary}
A square matrix $A$ has an inverse if and only if $\textnormal{rref}(A)=I$.
\end{corollary}
...
% The following exercises are from Kuttler, Problems 2.1.32-2.1.34.

% {\color{blue}These are pretty standard, I don't think we need to quote Kuttler.  I added two more problems. 17, 18}
