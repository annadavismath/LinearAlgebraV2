...
\author{Paul Zachlin \and Anna Davis} \title{SYS-0030:  Gaussian Elimination and Rank} \license{CC-BY 4.0}
\begin{document}

\begin{abstract}
 We introduce Gaussian elimination and Gauss-Jordan elimination algorithms, and define the \dfn{rank} of a matrix.
\end{abstract}
\maketitle
...
In module SYS-M-0020, we learned to write linear systems in \dfn{augmented matrix} form and use elementary row operations to carry an augmented matrix to \dfn{row-echelon form} and the \dfn{reduced row-echelon form} in order to solve linear systems.  
...
\begin{exploration}\label{explore:rrefunique} In this problem we revisit the system
$$\begin{array}{ccccccccc}
      x &+ &2y&-&3z&= &1 \\
	 -5x& +&2y&-&3z&=&1\\
      x&- &2y&+&z&=&1
...
The \dfn{rank} of matrix $A$ is the number of nonzero rows in any row-echelon matrix to which $A$ can be carried by elementary row operations.
\end{definition}
...
%The \dfn{rank} of a matrix (abbreviated \textnormal{rank}(A)) is the number of nonzero rows when a matrix is in row-echelon form.  Equivalently, \textnormal{rank}(A) is the number of pivots in either of the above algorithms.
%\end{definition}

% It can be shown that what IS unique is the positioning of the pivots in the matrix.  \textcolor{red}{Maybe we do this in a module VSP-0045?}
% It turns out that the reduced row-echlon form of any matrix is unique.  \textcolor{red}{Maybe we do this in a module VSP-0045?}

% The uniqueness of the positions of the pivots allows us to 
%Consider the system
%$$\begin{array}{ccccccccc}
 %     2x &+ &4y&= &3 \\
%	 -x&+ &y&=&-2\\
...
\begin{selectAll}
 ...
 \choice{Any row-echelon form of $A$ will have at least $5$ nonzero rows}
 \choice{Any row-echelon form of $A$ will have at most $5$ nonzero rows}
 \end{selectAll}
\end{problem}
