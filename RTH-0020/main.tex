...
\filldraw[blue, opacity=0.3] (2,0,2)--(2,0,-2)--(-2,0,-2)--(-2,0,2)--cycle;
    ...
    %\draw[-,line width=0.4mm, -stealth](0,0,0)--(0.8, -.8, -2) ;
    
    \node[fill,circle,inner sep=1.5pt,label={left:$O$}] at (0,0,0) {};
     \end{tikzpicture}
...
Next we consider the set of vectors that are orthogonal to \underline{every} vector in a given subspace $W$. 

\section*{Orthogonal Complements}
\label{sec:8_1}

\begin{definition}[Orthogonal Complement of a Subspace of $\RR^n$]\label{def:023776}
If $W$ is a subspace of $\RR^n$, define the \textbf{orthogonal complement} $W^\perp$ of $W$ (pronounced ``$W$-perp'') by
\begin{equation*}
