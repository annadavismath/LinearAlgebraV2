...
\author{Anna Davis \and Rosemarie Emanuele \and Paul Bender} \title{Linear Combinations of Vectors} \license{CC-BY 4.0}
...
The following GeoGebra interactive will allow you to go through the steps given in Procedure \ref{pro:lincombgeo} for a combination of vectors of your choice.
...
Visualizing linear combination of vectors in $\RR^3$ is more difficult than doing so in $\RR^2$.  The following GeoGebra interactive will help you do this.

\begin{exploration}\label{exp:vecAddInR3}
Recall that adding two vectors amounts to finding the diagonal of a parallelogram determined by placing the two vectors tail to tail.  This process is not limited to vectors of $\RR^2$.  In the following exercise you will define vectors $\vec{v}$, $\vec{w}$ and $\vec{u}$, then add them by finding diagonals of parallelograms, two vectors at a time. RIGHT-CLICK and DRAG the the left panel to rotate the graph.

\begin{center} 
\geogebra{ccpev33m}{900}{800} 
\end{center}

The process we visualized above for three vectors allows us to add any number of vectors, two at a time.  A special case that you will find useful is adding three vectors in $\RR^3$.  It turns out that the sum of three (non-co-planar) vectors is the diagonal of a three-dimensional figure called a \emph{parallelepiped}.  Each face of the parallelepiped is a parallelogram determined by two out of the three given vectors.  The following GeoGebra exercise will help you visualize the sum of three vectors as the diagonal of a parallelepiped.

\begin{center} 
\geogebra{bpzfb7vr}{900}{700} 
\end{center}

Finally, you can use the following GeoGebra interactive to help you visualize linear combinations of three vectors in $\RR^3$.
