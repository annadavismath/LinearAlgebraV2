...
We have used the dot product in $\RR^n$ to compute the length of vectors (Corollary \ref{cor:length_via_dotprod}) and also the \href{https://ximera.osu.edu/oerlinalg/LinearAlgebra/VEC-0060/main}{angle between vectors}. The plan in this section is to define an \dfn{inner product} on an arbitrary real vector space $V$ (of which the dot product is an example in $\RR^n$) and use it to introduce these concepts in $V$.
