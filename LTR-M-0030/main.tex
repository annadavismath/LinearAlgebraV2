...
\draw [->,line width=1pt,-stealth] (7,1)to[out=60](9.5, 1)node[above=7mm, left=6mm]{$S$};
\draw [->,line width=1pt,-stealth] (2,3.5)to[out=10](11, 3.5)node[above=12mm, left=20mm]{$S\circ T$};
\end{tikzpicture}
...
We conclude this section by revisiting the associative property of matrix multiplication.  At the time matrix multiplication was introduced, we found it extremely cumbersome to prove that for appropriately sized matrices $A$, $B$ and $C$, we have $(AB)C=A(BC)$ {\color{red} need reference}.  We are now in a position to prove this result with ease.  
...
\begin{proof} The proof of this result is left to the reader.
