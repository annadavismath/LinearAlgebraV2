...
\author{Anna Davis \and Paul Zachlin} \title{LTR-0030:  Composition and Inverses of Linear Transformations} \license{CC-BY 4.0}
...
Definition \ref{def:inverseoflintrans} does not specifically require an inverse $S$ of a linear transformation $T$ to be linear.  This is because the requirement that $S\circ T=\id_V$ and $T\circ S=\id_W$ is sufficient to guarantee that $S$ is linear.  
...
\end{exploration}
This Exploration highlights the necessity of a further discussion of inverses.  In Example \ref{ex:subtosub} of LTR-0035, we will prove that $T$ has an inverse, and in Example \ref{ex:inversematrixoftransform} of LTR-0080 we will relate the existence of an inverse of $T$ to a matrix associated with $T$.
