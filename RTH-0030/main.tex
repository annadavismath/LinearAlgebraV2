...
\title{Best Approximation and Least-Squares} \license{CC BY-NC-SA 4.0}
...
\end{exploration}
  ...
  In the case when $\text{col}(A)$ is a subspace of $\RR^3$, it is easy to see that $\vec{z}$ is the best approximation if and only if $A\vec{z}$ is an orthogonal projection of $\vec{b}$ onto $\text{col}(A)$, and the error is the magnitude of $\left(A\vec{z}\right)^{\perp}=\vec{b}-A\vec{z}$, as shown below.
...
What we observed for a $3\times 2$ matrix $A$, holds in general.  We will use this fact to find $\vec{z}$.
...
Observe that the columns of $A$ are linearly independent, so $(A^TA)^{-1}$ exists.  Applying equation (\ref{eq:leastSquaresZ}), we compute
\begin{eqnarray*}
...
\begin{equation}
    ...
    \end{bmatrix}
\end{equation}
Observe that each entry of $\vec{b}-A\vec{z}$ is the signed vertical distance between the point and the line.
