...
We are familiar with two operations that can be applied to vectors in $\RR^n$, namely, addition and scalar multiplication. We learned that addition and scalar multiplication satisfy many nice properties (see Theorem \ref{th:vecproperties}). These properties give $\RR^n$ an algebraic structure, and make $\RR^n$ a \dfn{vector space}.  Some subsets of $\RR^n$, such as lines and planes that pass through the origin, are vector spaces in their own right.  We will refer to such spaces as \dfn{subspaces} of $\RR^n$.  Before we can give formal definitions of a vector space and a subspace we need to introduce the concept of \dfn{closure} under an operation.
