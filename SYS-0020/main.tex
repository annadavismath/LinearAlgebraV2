...
$$\left[\begin{array}{c|c}  
 A & \vec{b}\\
 \end{array}\right]$$
The same elementary row operations that we perform on a system of equations can be performed on a matrix (or an augmented matrix).  If a matrix (augmented matrix) was obtained from another matrix (augmented matrix) by means of elementary row operations, we say that the two matrices are \dfn{row-equivalent}.  
