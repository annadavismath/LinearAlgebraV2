...
\end{eqnarray*}
The reason we included {\color{red}$\clubsuit$} in the expression is because we know that we cannot subtract $\lambda$, a scalar, from a matrix.  We need to replace {\color{red}$\clubsuit$} with an expression such that (a) $\lambda {\color{red}\clubsuit}$ is a $2\times 2$ matrix, and (b) $\lambda {\color{red}\clubsuit}\vec{x}=\lambda\vec{x}$. To make this happen, we should replace {\color{red}$\clubsuit$} with \wordChoice{\choice{$A$} \choice[correct]{$I$} \choice{1}\choice{the zero vector}}
...
The equation 
\begin{equation}\mbox{det}(A-\lambda I) = 0\end{equation}\label{eqn:chareqn} 
