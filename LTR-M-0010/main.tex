...
The \dfn{identity transformation} on $\RR^n$, denoted by $\id_{\RR^n}$, is a transformation that maps each element of $\RR^n$ to itself.

In other words,
$$\id_{\RR^n}:\RR^n\rightarrow \RR^n$$ is a transformation such that $$\id_{\RR^n}(\vec{v})=\vec{v}\quad\text{for all}\quad \vec{v} \in \RR^n$$
\end{definition}
...
In other words,
$$Z:\RR^n\rightarrow \RR^m$$ is a transformation such that $$Z(\vec{v})=\vec{0}\quad\text{for all}\quad \vec{v} \in \RR^n$$
\end{definition}
