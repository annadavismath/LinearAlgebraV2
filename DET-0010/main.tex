...
\begin{corollary}\label{lemma:detofid} Let $I$ be the identity matrix, then
 ...
 We first introduced block matrices in \href{https://ximera.osu.edu/oerlinalg/LinearAlgebra/MAT-0023/main}{Block Matrix Multiplication}.  Matrices of the form $\begin{bmatrix}A & C\\\vec{O}& B\end{bmatrix}$ and $\begin{bmatrix}A & \vec{O}\\D& B\end{bmatrix}$, where $A$ and $B$ are square matrices, are said to be \dfn{block triangular}.  The following theorem makes it easy to compute determinants of such matrices.

 \begin{theorem}
     Consider block triangular matrices $\begin{bmatrix}A & C\\\vec{O}& B\end{bmatrix}$ and $\begin{bmatrix}A & \vec{O}\\D& B\end{bmatrix}$, where $A$ and $B$ are square matrices.  Then
     $$\det{\begin{bmatrix}A & C\\\vec{O}& B\end{bmatrix}}=\det{A}\det{B}\quad\mbox{and}\quad\det{\begin{bmatrix}A & \vec{O}\\D& B\end{bmatrix}}=\det{A}\det{B}$$
 \end{theorem}
\begin{proof}
    Write $T=\begin{bmatrix}A & C\\\vec{O}& B\end{bmatrix}$ and proceed by induction on $k$, where $A$ is $k\times k$.  If $k=1$, then the result follows from cofactor expansion along the first column.  In general, let $S_i(T)$ denote the matrix obtained from $T$ by deleting row $i$ and column 1. Then the cofactor expansion along the first column is
    ...
    $$S_i(T)=\begin{bmatrix}S_i(A) & C_i\\\vec{O}& B\end{bmatrix}$$
