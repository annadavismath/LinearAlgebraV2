...
%This shows that if we had mistakenly used $\mathcal{S}$ as a basis for $V$, the vector $\begin{bmatrix}1\\1\\3\end{bmatrix}$ would not have a unique coordinate vector with respect to the proposed basis.  Indeed, from the equations in \ref{two_ways}, we may arrive at these coordinate vectors with respect to $\mathcal{S}$:
