...
Recall that every basis of $\RR^n$ (or a subspace $W$ of $\RR^n$) imposes a coordinate system on $\RR^n$ (or $W$) that can be used to express any vector of $\RR^n$ (or $W$) as a linear combination of the elements of the basis.  For example, vectors $\vec{v}_1$ %=\begin{bmatrix}1\\2\end{bmatrix}$ 
...
is an orthogonal set of vectors.  Use this fact to write $\vec{x}$ as  a linear combination of the vectors of $\mathcal{B}$.
...
In the previous section we had found that given a subspace $W$ of $\RR^n$ with orthogonal basis $\mathcal{B}$, every vector $\vec{x}$ in $W$ can be expressed as the sum of the orthogonal projections of $\vec{x}$ onto the elements of $\mathcal{B}$.  Note that our premise was that $\vec{x}$ is in $W$.  In this section, we look into the meaning of the sum of orthogonal projections of $\vec{x}$ onto the elements of an orthogonal basis of $W$ for those vectors $\vec{x}$ of $\RR^n$ that are \emph{not} in $W$.

\begin{exploration}\label{exp:orthProjSub}
In the GeoGebra interactive below, $W$ is a plane, spanned by $\vec{v}_1$ and $\vec{v}_2$, in $\RR^3$.  $W$ is subspace of $\RR^3$.  In the initial set up, $\vec{v}_1$ and $\vec{v}_2$ are orthogonal.  Vector $\vec{x}$ is not in $W$.  Use check-boxes to construct the sum of orthogonal projections of $\vec{x}$ onto $\vec{v}_1$ and $\vec{v}_2$.  RIGHT-CLICK and DRAG to rotate the image.   
    \begin{center}
\geogebra{hehqyayz}{950}{800}
\end{center}
\begin{enumerate}
\item If moved, return the basis vectors $\vec{v}_1$ and $\vec{v}_2$ to their default position ($s_1=s_2=0$) to ensure that they are orthogonal.  Rotate the image to convince yourself that the perpendiculars dropped from the tip of $\vec{x}$ to $\vec{v}_1$ and $\vec{v}_2$ appear perpendicular to $\vec{v}_1$ and $\vec{v}_2$ in the diagram. (You will have to find a good vantage point to convince yourself.)  Are both of these perpendiculars also necessarily perpendicular to the plane? \wordChoice{\choice{Yes}, \choice[correct]{No}}
 ...
 \end{multipleChoice}
 Rotate your figure so that you're looking directly down at the plane, as you did in the previous part.  Which of the following is true?
 \begin{multipleChoice}
 \choice{the parallelogram determined by $\vec{v}_1$ and $\vec{v}_2$ is a rectangle.}
...
In Exploration \ref{exp:orthProjSub}, you had discovered that given a plane, spanned by orthogonal vectors $\vec{v}_1,\vec{v}_2$, in $\RR^3$ , and a vector $\vec{x}$, not in the plane, we can interpret the sum of orthogonal projections of $\vec{x}$ onto $\vec{v}_1$ and $\vec{v}_2$ as a ``shadow" of $\vec{x}$ that lies in the plane directly underneath the vector $\vec{x}$. We say that this ``shadow" is an orthogonal projection of $\vec{x}$ onto $W$. You have also found that if $\vec{v}_1,\vec{v}_2$ are not orthogonal, the ``shadow" of $\vec{x}$ no longer lies directly underneath $\vec{x}$.  Therefore, it is crucial that $\vec{v}_1,\vec{v}_2$ are orthogonal for the ``shadow" to be considered an orthogonal projection.  
...
\draw[-,line width=0.2mm, dashed](4,3,4)--(4,0,4) ;
   ...
   \draw[->,line width=0.8mm, -stealth, red](0,0,0)--(4,3,4) ;
    ...
    \node[label={below:$\vec{w}=\mbox{proj}_V\vec{x}=\mbox{proj}_{\vec{f}_1}\vec{x}+\mbox{proj}_{\vec{f}_2}\vec{x}$}] at (3,-1,3) {};
      \end{tikzpicture}
\end{center}
...
Definition \ref{def:projOntoSubspace} allows us to express $\vec{x}$ as the sum of its orthogonal projection, $\vec{w}=\mbox{proj}_V(\vec{x})$ located in $W$, and a vector we will call $\vec{w}^\perp$ (pronounced ``W-perp"), given by $\vec{w}^\perp=\vec{x}-\vec{w}$. This decomposition of $\vec{x}$ is shown in the diagram below.  
\begin{center}
\tdplotsetmaincoords{70}{130}
	\begin{tikzpicture}[scale=0.8]
\filldraw[blue, opacity=0.2] (0,0,0)--(5,0,0)--(5,0,5)--(0,0,5)--cycle;
...
\draw[->,line width=1.5mm, -stealth, red,opacity=0.2](4,0,4)--(4,3,4) ;
    ...
    \node[label={above:$W$}] at (5,0,1.9) {};
     ...
     \end{center}
In Exploration \ref{exp:orthProjSub}, you had met $\vec{w}^\perp$ under the name of $\vec{p}$ and observed that this vector is orthogonal to $W$. 
The fact that this decomposition is unique and that $\vec{w}^\perp$ is orthogonal to every vector in $W$ is the subject of the Orthogonal Decomposition Theorem which we will prove in the next section.
