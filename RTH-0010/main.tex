...
\author{Paul Zachlin \and Anna Davis} \title{Orthogonal and Orthonormal Sets} \license{CC-BY 4.0}
...
Recall that every basis of $\RR^n$ (or a subspace $V$ of $\RR^n$) imposes a coordinate system on $\RR^n$ (or $V$) that can be used to express any vector of $\RR^n$ (or $V$) as a linear combination of the elements of the basis.  For example, vectors $\vec{v}_1$ %=\begin{bmatrix}1\\2\end{bmatrix}$ 
and $\vec{v}_2$ %=\begin{bmatrix}3\\1\end{bmatrix}$ 
impose a coordinate system onto the plane, as shown in the figure below.  We readily see that $\vec{x}$, contained in the plane, %=\begin{bmatrix}7\\4\end{bmatrix}$ 
...
\begin{enumerate}
 ...
 \choice{Vector $\vec{p}$ is orthogonal to $V$.}
 \choice{Vector $\mbox{proj}_{\vec{v}_1}\vec{x}+\mbox{proj}_{\vec{v}_2}\vec{x}$ is a projection of $\vec{x}$ onto $V$.}
...
%We have studied the orthogonal projection of one vector onto another, and made extensive use of the formula
%\begin{equation}
%\mbox{proj}_{\vec{d}}\vec{x}=\left(\frac{\vec{x}\cdot\vec{d}}{\norm{\vec{d}}^2}\right)\vec{d}.
%\end{equation}\label{def:projectontod}
...
\filldraw[blue, opacity=0.3] (0,0,0)--(0,0,5)--(0,3,5)--(0,3,0)--cycle;
\filldraw[blue, opacity=0.2] (0,0,0)--(5,0,0)--(5,3,0)--(0,3,0)--cycle;
\filldraw[blue, opacity=0.4] (0,0,0)--(5,0,0)--(5,0,5)--(0,0,5)--cycle;
\draw[->,line width=0.4mm, -stealth, blue](0,0,0)--(6,0,0) ;
\draw[->,line width=0.4mm, -stealth, blue](0,0,0)--(0,4,0) ;
\draw[->,line width=0.4mm, -stealth, blue](0,0,0)--(0,0,6) ;
\node[label={above:$\vec{f}_1$}] at (6,0,0) {};
\node[label={right:$\vec{f}_2$}] at (0,4,0) {};
\node[label={left:$\vec{f}_3$}] at (0,0,6) {};
\node[label={above:$\vec{x}$}] at (2,1.1,2) {};
\node[label={below:$\vec{x}=\mbox{proj}_{\vec{f}_1}\vec{x}+\mbox{proj}_{\vec{f}_2}\vec{x}+\mbox{proj}_{\vec{f}_3}\vec{x}$}] at (3,-1,4) {};
    \draw[->,line width=0.8mm, -stealth, red](0,0,0)--(4,2.5,4) ;
    \draw[->,line width=1.5mm, -stealth, black,opacity=0.3](0,0,0)--(0,0,4) ;
    \draw[->,line width=1.5mm, -stealth, black,opacity=0.3](0,0,0)--(4,0,0) ;
    \draw[->,line width=1.5mm, -stealth, black,opacity=0.3](0,0,0)--(0,2.5,0) ;
    
    \draw[-,line width=0.2mm, dashed](4,2.5,4)--(4,0,4) ;
    \draw[-,line width=0.2mm, dashed](4,2.5,4)--(0,2.5,4) ;
    \draw[-,line width=0.2mm, dashed](4,2.5,4)--(4,2.5,0) ;

    \draw[-,line width=0.2mm, dashed](0,0,4)--(4,0,4) ;
    \draw[-,line width=0.2mm, dashed](0,0,4)--(0,2.5,4) ;
    \draw[-,line width=0.2mm, dashed](4,0,4)--(4,0,0) ;
    \draw[-,line width=0.2mm, dashed](4,2.5,0)--(4,0,0) ;
    \draw[-,line width=0.2mm, dashed](4,2.5,0)--(0,2.5,0) ;
    \draw[-,line width=0.2mm, dashed](0,2.5,4)--(0,2.5,0) ;
        \end{tikzpicture}
\end{center}

Now consider subspaces $U=\mbox{span}(\vec{f}_1,\vec{f}_2)$, $V=\mbox{span}(\vec{f}_1,\vec{f}_3)$ and $W=\mbox{span}(\vec{f}_2,\vec{f}_3)$ of $\RR^3$.  Applying the same intuition to the planes $U, V$ and $W$ as we did to vectors (lines), we can speak of projections of $\vec{x}$ onto $U, V$ and $W$.  These projections are shown in the figure below.
...
\node[label={right:$\vec{f}_2$}] at (0,4,0) {};
\node[label={left:$\vec{f}_3$}] at (0,0,6) {};
\node[label={above:$\vec{x}$}] at (2,0.6,2) {};
\node[label={above:$U=\mbox{span}(\vec{f}_1,\vec{f}_2)$}] at (4.7,2.8,0) {};
\node[label={above:$V=\mbox{span}(\vec{f}_1,\vec{f}_3)$}] at (6.5,-0.2,5) {};
\node[label={above:$W=\mbox{span}(\vec{f}_2,\vec{f}_3)$}] at (-2,2.5,1) {};
    \draw[->,line width=0.8mm, -stealth, red](0,0,0)--(4,2.5,4) ;
    
    \draw[->,line width=1.5mm, -stealth, black,opacity=0.2](0,0,0)--(0,2.5,4) ;
    \draw[->,line width=1.5mm, -stealth, black,opacity=0.2](0,0,0)--(4,0,4) ;
    \draw[->,line width=1.5mm, -stealth, black,opacity=0.2](0,0,0)--(4,2.5,0) ;
    \draw[-,line width=0.2mm, dashed](4,2.5,4)--(4,0,4) ;
    \draw[-,line width=0.2mm, dashed](4,2.5,4)--(0,2.5,4) ;
    \draw[-,line width=0.2mm, dashed](4,2.5,4)--(4,2.5,0) ;
...
We will use the notation $\mbox{proj}_U\vec{x}$, $\mbox{proj}_V\vec{x}$ and $\mbox{proj}_W\vec{x}$ to to denote projections of $\vec{x}$ onto $U, V$ and $W$, respectively.  Such projections are called \emph{projections onto a subspace} of $\RR^n$.  In general, subspaces can be lines, planes or hyperplanes.  For instance, a projection of a vector onto another vector is simply a projection onto a one-dimensional subspace of $\RR^n$.
Looking at the above diagram, it is intuitively clear that $$\mbox{proj}_U\vec{x}=\mbox{proj}_{\vec{f}_1}\vec{x}+\mbox{proj}_{\vec{f}_2}\vec{x}$$
$$\mbox{proj}_V\vec{x}=\mbox{proj}_{\vec{f}_1}\vec{x}+\mbox{proj}_{\vec{f}_3}\vec{x}$$
$$\mbox{proj}_W\vec{x}=\mbox{proj}_{\vec{f}_2}\vec{x}+\mbox{proj}_{\vec{f}_3}\vec{x}$$

We will often find it convenient to express a vector as a sum of its projection onto a subspace and a vector orthogonal to the subspace.  The following diagram demonstrates this decomposition for vector $\vec{x}$ and its projection $\vec{w}$ onto $V$.  In the diagram, $\vec{w}=\mbox{proj}_{\vec{f}_1}\vec{x}+\mbox{proj}_{\vec{f}_3}\vec{x}$, and $\vec{w}^{\perp}$ is a vector orthogonal to $\vec{w}$ defined by $\vec{w}^{\perp}=\vec{x}-\vec{w}$.
...
\node[label={above:$\vec{w}^{\perp}$}] at (5.3,1.5,6) {};
    \draw[->,line width=0.8mm, -stealth, red](0,0,0)--(4,2.5,4) ;
    
    \draw[->,line width=1.5mm, -stealth, black,opacity=0.2](0,0,0)--(4,0,4) ;

    \draw[->,line width=1.5mm, -stealth, black,opacity=0.2](4,0,4)--(4,2.5,4) ;
    \draw[-,line width=0.2mm, dashed](4,2.5,4)--(0,2.5,4) ;
    \draw[-,line width=0.2mm, dashed](4,2.5,4)--(4,2.5,0) ;
...
These ideas are generalized in the following definition.
