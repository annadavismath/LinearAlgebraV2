...
In this section we will establish the formula for the distance between two points in $\RR^n$.  We begin by observing that the distance between two numbers (points) $x_1$ and $x_2$ on a number line is given by $|x_1-x_2|$.  (Why do we use the absolute value brackets?).
...
Let $A(x_1, y_1, z_1)$ and $B(x_2, y_2, z_2)$ be points in $\RR^3$.  Then the distance between $A$ and $B$ is given by
...
Let $A(a_1, a_2,\ldots ,a_n)$ and $B(b_1, b_2,\ldots ,b_n)$ be points in $\RR^n$.  Then the distance between $A$ and $B$ is given by
