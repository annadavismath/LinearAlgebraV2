...
Find $\det{B}$.
$$\det{B}=\answer{-21}$$
Next, let's switch consecutive rows.  Construct matrix $B'$ by interchanging the second and third rows of $A$.
...
The following lemma is a useful consequence of parts \ref{item:rowswapanddet} and  \ref{item:rowswapanddet} of Theorem \ref{th:elemrowopsanddet}.
...
% \end{array}\right]
\end{align}
We stop at a row-echelon form, $A'$, of $A$ because its determinant is the product of its diagonal entries (See Practice Problem \ref{prob:detoftrimat}).
\begin{align}\label{eq:detofrefa}\det{\big(A'\big)}=\frac{1}{5}\end{align}
...
$$\begin{array}{|c|c|}  
 ...
 \hline A'& \frac{1}{5}\cdot\frac{1}{2}\cdot\frac{1}{3}\det{A} \\ \hline
 \end{array}$$
