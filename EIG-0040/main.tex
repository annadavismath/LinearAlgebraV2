...
As we will see, matrices that are similar share many properties.  The first three properties contained in the following theorem are important enough in mathematics to receive a name.  We say that any relation which is \dfn{reflexive}, \dfn{symmetric}, and \dfn{transitive}, is called an \dfn{equivalence relation}.
...
This proves that similarity $\sim$ between matrices is an \dfn{equivalence relation}.  Practice Problem \ref{prob:lessthan} gives a good example of a relation that is NOT an equivalence relation.


We pause to introduce a simple matrix function that will be referred to later.  Given a square matrix, we have learned how to compute the determinant.  Another important quantity we can compute is the \dfn{trace}.
...
At the beginning of this module we mentioned that similarity of $n \times n$ matrices is an \dfn{equivalence relation}.
...
\begin{problem}\label{prob:notsimilar}
