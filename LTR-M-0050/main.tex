...
\begin{tikzpicture}[scale=.7]
  ...
  \draw[<->] (4,0)--(8,0);
  \draw[<->] (5,-1)--(5,3);
  \draw[line width=2pt,blue](4.5,-1)--(6.5,3) node[below=5mm, left=-10mm]{$\mbox{im}(T)$};
...
% Recall that the span of any collection of vectors of $\RR^m$ is a subspace of $\RR^m$ {\color{red} insert reference}.  Thus, $\mbox{im}(T)$ is a subspace of $\RR^m$. This justifies the following definition.
...
\draw[gray!40,line width=1pt](0.3,2.4)node[below=8mm, left=4mm][black]{$V$}--(5,1.1)node[below=5mm, right=-6mm][black]{$\mbox{im}(T)$};
...
Notice that since the nullity of a matrix $A$ was defined as $\mbox{nullity}(A) = \mbox{dim}(\mbox{null}(A))$ {\color{red} need reference}, and the kernel of a linear transformation $T:\RR^n\rightarrow \RR^m$ is the null space of its associated standard matrix, our definitions of nullity are in agreement.
